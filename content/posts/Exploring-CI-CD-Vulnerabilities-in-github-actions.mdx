...
This workflow will run after the workflow named `Publish Package'` has been successfully completed as mentioned in the `if:`. So to trigger this
workflow we need to create the workflow name `Publish Package` in the fork repo [there can be the same workflow with the same name]. 

# Exploitation 

here will create a workflow that will have the name `Publish Package` with a normal `pull_request` event, Which will successfully run and trigger the vulnerable 
workflow
```yaml
...
Assign write permission where it's needed.
